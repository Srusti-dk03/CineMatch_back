...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Error creating account. Email might be in use.');
    }
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] p-4 relative overflow-hidden">
      ...
      >
        <h2 className="text-4xl font-extrabold mb-6 text-center text-gray-900 dark:text-white tracking-tight">Create Account</h2>
        
        {error && <div className="bg-red-50 text-red-600 font-bold p-3 rounded-lg mb-6 text-sm text-center">{error}</div>}
        
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            <label className="text-md font-bold text-gray-900 dark:text-gray-100">Username</label>
            <div className="relative">
              <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-primary-500 h-6 w-6" />
              <input 
                type="text" 
                value={username}
          ...
          <button 
            type="submit" 
            className="w-full bg-primary-600 hover:bg-primary-500 text-white font-extrabold text-lg py-4 px-4 rounded-xl transition-all flex justify-center items-center space-x-2 shadow-lg shadow-primary-500/30 mt-4"
          >
            <span>Sign Up</span>
        ...
        <p className="mt-6 text-center text-gray-600 dark:text-gray-300 font-medium">
          ...
          <Link to="/login" className="text-primary-600 dark:text-primary-400 font-extrabold hover:underline transition-colors">Log in</Link>
        </p>
      </motion.div>
    </div>
  );
